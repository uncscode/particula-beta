...
def get_new_files(
    ...
    file_info = [[name, size] for name, size in zip(file_names, file_sizes)]
...
def load_files_interface(
    ...
    full_paths, first_pass, file_info = get_new_files(
        path=path, import_settings=settings, loaded_list=stream.files
    )
...
def load_folders_interface(
    ...
    for key, file_settings in folder_settings.items():
        ...
        lake[key] = load_files_interface(
            path=path, settings=file_settings, stream=stream
        )
    return lake
...
def get_1d_stream(
    ...
    if not os.path.isfile(file_path):
        raise FileNotFoundError(
            f"The file path specified does not exist: \
                                {file_path}"
        )
    ...
    if "date_location" in settings:
        date_offset = loader.non_standard_date_location(
            data=data, date_location=settings["date_location"]
        )
    else:
        date_offset = None
