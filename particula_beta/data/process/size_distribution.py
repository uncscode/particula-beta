...
def mean_properties(
    ...
    if sizer_limits is None:
        ...
        threshold_limits = (sizer_diameter >= sizer_limits[0]) & (
            sizer_diameter <= sizer_limits[1]
        )  # gets indexes to keep
    ...
    mode_diameter = np.interp(
        0.5, sizer_dn_cumsum, sizer_diameter, left=np.nan, right=np.nan
    )

    # Calculate mode diameter by mass.
    sizer_dmass_cumsum = np.cumsum(mass_ug_m3) / unit_mass_ug_m3
    mode_diameter_mass = np.interp(
        0.5, sizer_dmass_cumsum, sizer_diameter, left=np.nan, right=np.nan
    )
    return (
        total_concentration,
        unit_mass_ug_m3,
...
def sizer_mean_properties(
    ...
    """
    sizer_diameter_smps = np.array(stream.header).astype(
        float
    ...
    for i in range(len(stream.time)):
        (
            total_concentration[i],
            unit_mass_ug_m3[i],
            mean_diameter_nm[i],
            mean_vol_diameter_nm[i],
            geometric_mean_diameter_nm[i],
            mode_diameter[i],
            mode_diameter_mass[i],
        ) = mean_properties(
...
def merge_size_distribution(
    ...
    diameters_lower = np.array(stream_lower.header).astype(
        float
    ) * convert_units(lower_units, "nm")

    diameters_upper = np.array(stream_upper.header).astype(
        float
