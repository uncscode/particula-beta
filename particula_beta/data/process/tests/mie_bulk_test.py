...
def test_mie_return_type():
    ...
    result = mie_bulk.discretize_auto_mieq(
        m_sphere=1.5 + 0.1j, wavelength=550, diameter=100
    )
    assert isinstance(result, tuple), "Result should be a tuple"
    assert all(
        isinstance(x, float) for x in result
    ), "All items in the tuple should be floats"
...
def test_mie_valid_inputs():
    ...
    assert result == pytest.approx(
        expected, abs=1e-4
    ), "The function output for valid inputs"
...
def test_mie_edge_cases():
    ...
    result = mie_bulk.discretize_auto_mieq(
        m_sphere=1.5 + 0.1j, wavelength=550, diameter=1
    )
...
def test_discretize_with_custom_bases():
    ...
    assert m_sphere == pytest.approx(expected_m_sphere), "Custom base m_sphere"
    assert wavelength == pytest.approx(
        expected_wavelength
    ), "Custom base wavelength"
    assert all(
        d == pytest.approx(d_val)
...
def test_discretize_diameter_float():
    ...
    assert m_sphere == pytest.approx(
        expected_m_sphere
    ), "Custom base m_sphere was not applied correctly"

    # Check if the discretized wavelength matches the expected value
    assert wavelength == pytest.approx(
        expected_wavelength
    ), "Custom base wavelength was not applied right"
    ...
    assert diameter == pytest.approx(
        expected_diameter
    ), "Custom base diameter was not applied correctly"
...
def test_format_mie_results_as_dict():
    ...
    results = mie_bulk.format_mie_results(
        b_ext, b_sca, b_abs, big_g, b_pr, b_back, b_ratio, as_dict=True
    )
    ...
    assert np.array_equal(
        results["b_ratio"], b_ratio
    ), "b_ratio values mismatch"
...
def test_format_mie_results_as_tuple():
    ...
    results = mie_bulk.format_mie_results(
        b_ext, b_sca, b_abs, big_g, b_pr, b_back, b_ratio, as_dict=False
    )
    ...
    assert np.array_equal(
        results[5], b_back
    ), "b_back values mismatch in tuple"
    assert np.array_equal(
        results[6], b_ratio
    ), "b_ratio values mismatch in tuple"
...
def test_mie_size_distribution_extinction_only_tuple_output():
    ...
    assert result_ext == pytest.approx(ext_expected), "Expected extinction "
