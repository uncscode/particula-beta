...
def test_wrapped_cube():
    ...
    result = boundary.wrapped_cube(position=position, cube_side=cube_side)
    ...
    result = boundary.wrapped_cube(position, cube_side)
    assert torch.allclose(result, expected_result)
    ...
    assert torch.allclose(
        boundary.wrapped_cube(position, cube_side), expected_result
    )
